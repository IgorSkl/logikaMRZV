/***********************************************************************
 * $Id: lpc32xx_hsuart_driver.c 35145 2010-03-19 18:10:17Z anderslu $
 *
...
#include "../include/lpc32xx_uart.h"
#include "../include/lpc32xx_hsuart_driver.h"
#include "../include/lpc32xx_intc_driver.h"
#include "../include/lpc32xx_clkpwr_driver.h"
#include "../include/lpc32xx_timer_driver.h"
...
extern unsigned long lTmrHSU2Val;
/* HSUART driver data */
static HSUART_CFG_T hs_uart_dat [3];
 ...
 *
 **********************************************************************/
void hs_uart_gen_int_handler(HSUART_CFG_T *phsuartcfg)
{
  volatile UNS_32 tmp;
  long il;
  ...
  if ((tmp & (HSU_RX_TRIG_INT | HSU_RX_TIMEOUT_INT)) != 0)
  {
    ...
    if (phsuartcfg->cbs.rxcb != NULL)
    {
      phsuartcfg->cbs.rxcb(&phsuartcfg);
	    ...
	    if(tmp & HSU_RX_TIMEOUT_INT)
	    {
		  if (phsuartcfg->cbs.eofcb != NULL)
		  phsuartcfg->cbs.eofcb();
	    }
	  il++;
    }
    else
  ...
  if ((tmp & (HSU_RX_OE_INT | HSU_BRK_INT | HSU_FE_INT)) != 0)
  {
	...
	if(il==0) return;//Frease on receive
	il = hs_uart_ptr_to_hsuart_num(phsuartcfg->regptr);
	switch (il )
	{
		case 0:
 ...
 *
 **********************************************************************/
void hs_uart1_int_handler(void)
{
  //hs_uart_gen_int_handler(&hsuartdat[0]);
  //int_disable(IRQ_UART_IIR1);
  hs_uart_gen_int_handler(&hs_uart_dat[0]);
 ...
 *
 **********************************************************************/
void hs_uart2_int_handler(void)
{
  //hs_uart_gen_int_handler(&hsuartdat[1]);
  //Int_disable(IRQ_UART_IIR2);
  hs_uart_gen_int_handler(&hs_uart_dat[1]);
 ...
 *
 **********************************************************************/
void hs_uart7_int_handler(void)
{
  //hsuart_gen_int_handler(&hsuartdat[2]);
  //disable_irq_fiq();
  hs_uart_gen_int_handler(&hs_uart_dat[2]);
 //enable_irq_fiq();
}
 ...
 *
 **********************************************************************/
INT_32 hs_uart_ptr_to_hsuart_num(HSUART_REGS_T *phsuart)
{
  INT_32 hsuartnum = -1;
 ...
 *
 **********************************************************************/
void hs_uart_flush_fifos(HSUART_REGS_T *pregs,
                        UNS_32 flushword)
{
  UNS_8 dummy = 0;
 ...
 *
 **********************************************************************/
UNS_32 hs_uart_find_clk(UNS_32 freq,
 ...
 *
 **********************************************************************/
STATUS hs_uart_setup_trans_mode(HSUART_CFG_T *phsuartcfg,
                               HSUART_CONTROL_T *phsuartsetup)
{
  STATUS err = _NO_ERROR;

  /* Find closest baud rate for desired clock frequency */
  phsuartcfg->baudrate = hs_uart_find_clk(phsuartsetup->baud_rate, phsuartcfg);
 ...
 *
 **********************************************************************/
INT_32 hs_uart_open(void *ipbase,
                   INT_32 arg)
{
  ...
  hsuartnum = hs_uart_ptr_to_hsuart_num((HSUART_REGS_T *) ipbase);
  if (hsuartnum >= 0)
  {
    /* Has the HS UART been previously initialized? */
    if (hs_uart_dat[hsuartnum].hsuart_init == FALSE)
    {
      /* HS UART is free */
      hs_uart_dat[hsuartnum].hsuart_init = TRUE;
      hs_uart_dat[hsuartnum].hsuartnum = hsuartnum;
      hs_uart_dat[hsuartnum].regptr = (HSUART_REGS_T *) ipbase;

      /* No callbacks by default */
      hs_uart_dat[hsuartnum].cbs.rxcb = NULL;
      hs_uart_dat[hsuartnum].cbs.txcb = NULL;
      hs_uart_dat[hsuartnum].cbs.rxerrcb = NULL;
      ...
      if (pucfg == NULL)
      {
        ...
        hs_uart_setup_trans_mode(&hs_uart_dat[hsuartnum], &hsuarttran);
      }
      else
      {
        hs_uart_setup_trans_mode(&hs_uart_dat[hsuartnum], pucfg);
      }

      /* Set FIFO level, and clear pending interrupts */
      hs_uart_dat[hsuartnum].regptr->ctrl = (HSU_HRTS_TRIG_32B | HSU_TMO_INACT_16B |
                                           HSU_OFFSET(0x14) | HSU_RX_TL32B | HSU_TX_TL0B);

      /* Receive, transmit and status interrupts enabled */
      hs_uart_dat[hsuartnum].regptr->ctrl |= HSU_RX_INT_EN | HSU_TX_INT_EN | HSU_ERR_INT_EN;

      /* Return pointer to specific UART structure */
      tptr = (INT_32) & hs_uart_dat[hsuartnum];
    }
  }
 ...
 *
 **********************************************************************/
STATUS hs_uart_close(INT_32 devid)
{
  volatile UNS_32 tmp;
 ...
 *
 **********************************************************************/
STATUS hs_uart_ioctl(INT_32 devid,
                    INT_32 cmd,
                    INT_32 arg)
{
  HSUART_CBS_T *pcbs;
  ...
  if (phsuart->hsuart_init == TRUE)
  {
    ...
    switch (cmd)
    {
      ...
      case HSUART_SETUP_TRANSFER:
        hs_uart_setup_trans_mode(phsuart, (HSUART_CONTROL_T *) arg);
        break;
 ...
 *
 **********************************************************************/
INT_32 hs_uart_read(INT_32 devid,
                   void *buffer,
                   INT_32 max_bytes)
{
  INT_32 bread = 0;
 ...
 *
 **********************************************************************/
INT_32 hs_uart_write(INT_32 devid,
                    void *buffer,
                    INT_32 n_bytes)
{
  INT_32 bwrite = 0;
...
//static long lMaxTimeTransmit;
INT_32 hs_uart_write1(INT_32 devid,
                    void *buffer,
                    INT_32 n_bytes) @ "Fast_function"
{
  INT_32 bwrite = 0;
  HSUART_CFG_T *phsuart = (HSUART_CFG_T *) devid;
  HSUART_REGS_T *pregs = phsuart->regptr;
  ...
  if (((pregs->level) >> 8) < 4)
  {
    while ((n_bytes > 0) && (((pregs->level) >> 8) < 63 ))
    {
      pregs->txrx_fifo = (UNS_32) * buff8;
      buff8++;
      n_bytes--;
      bwrite++;
		...
		n_bytes = 0;
		buffer = TIMER_CNTR0;
		lTmrHSU2Val = ((TIMER_CNTR_REGS_T*) buffer)->tc;
		}
    }
 ...
 *
 **********************************************************************/
STATUS  hs_uart_recover(INT_32 devid)
{
  volatile UNS_32 tmp;
  HSUART_CFG_T *phsuart = (HSUART_CFG_T *) devid;
  //INT_32 hsuartnum = 0;
  ...
  if (phsuart->hsuart_init == TRUE)
  {
    //hsuartnum = hs_uart_ptr_to_hsuart_num((HSUART_REGS_T *) devid);
	/* Disable interrupts */
    phsuart->regptr->ctrl &= ~(HSU_ERR_INT_EN | HSU_RX_INT_EN | HSU_TX_INT_EN);
	phsuart->regptr->ctrl = 0;
	 tmp = phsuart->regptr->rate;
	phsuart->regptr->rate = 0;
	phsuart->regptr->iir  = HSU_BRK_INT|HSU_FE_INT|HSU_RX_OE_INT|HSU_TX_INT ;
	phsuart->regptr->iir  = 0;

    /* Free UART and Disable timer system clock */
    //phsuart->hsuart_init = FALSE;
	dummy =	phsuart->regptr->txrx_fifo;
	hs_uart_ioctl(devid, HSUART_CLEAR_FIFOS, 0);
	
	phsuart->regptr->iir  = HSU_RX_TRIG_INT;
	phsuart->regptr->iir  = 0;
	phsuart->regptr->rate = tmp;
	phsuart->regptr->ctrl =   (HSU_TMO_INACT_16B |
    HSU_OFFSET(0x14) | HSU_RX_TL32B | HSU_TX_TL0B);//HSU_HRTS_TRIG_32B | 
	phsuart->regptr->ctrl |= HSU_RX_INT_EN |   HSU_TX_INT_EN | HSU_ERR_INT_EN;//HSU_RX_INT_EN |
