/***********************************************************************
 * $Id: lpc32xx_ssp_driver.c 30870 2009-07-05 10:16:16Z anderslu $
 *
...
#include "../include/lpc32xx_ssp_driver.h"
                 ...
                 INT_32 n_fifo)
{
  ...
  if (sspcfgptr->init == TRUE)
  {
    /* Loop until transmit ring buffer is full or until n_bytes
       expires */
	//devid = sspcfgptr->regptr->sr;
	if(sspcfgptr->regptr->sr&SSP_SR_RFF)
	devid = sspcfgptr->regptr->sr;
	
    while ((n_fifo > 0) &&
           ((sspcfgptr->regptr->sr & SSP_SR_TFE) != 0)&&//SSP_SR_TNF
		   ((sspcfgptr->regptr->sr & SSP_SR_RFF) == 0)
		   )
    {
      if (sspcfgptr->dsize == 1)
      {
