...
#define MNGUPR_PRM1    44
//уставка управления DV
#define MNGUPR_DVLOW   45
#define MNGUPR_DVMIDD  46
#define MNGUPR_DVHI    47
//уставка управления RELE
#define MNGUPR_RELE    48
//уставка управления CDI
#define MNGUPR_CDI     49

//уставки PRM1 блок откл
#define MNGUST_PRM1_BO  50
//уставки PRM1 блок вкл
#define MNGUST_PRM1_BV  51
//уставки PRM1 номин ток выключателя
#define MNGUST_PRM1_INOM_VV      52
//уставки PRM1 номин ток отключения выключателя
#define MNGUST_PRM1_INOM_OTKLVV  53
//уставки PRM1 ресурс номин токе выключателя
#define MNGUST_PRM1_NNOM_VV      54
//уставки PRM1 ресурс номин токе отключения выключателя
#define MNGUST_PRM1_NNOM_OTKLVV  55

//к-во уставок в менеджере уставок
#define MNGUST_PRM1_TOTAL    56
...
//ресурс выключателя
//уставки PRM1 номин ток выключателя
#define SPV_INOM_VV      75
//уставки PRM1 номин ток отключения выключателя
#define SPV_INOM_OTKLVV  76
//уставки PRM1 ресурс номин токе выключателя
#define SPV_NNOM_VV      77
//уставки PRM1 ресурс номин токе отключения выключателя
#define SPV_NNOM_OTKLVV  78

//сброс блока включения
#define SPV_RESET_BV     79

//к-во уставок в супервизоре
#define SPVUST_PRM1_TOTAL  80
...
//Привод ВВ
#define IOCMD14_PRM1 13

//к-во IO команд
#define IOCMD_PRM1_TOTAL    14
...
extern int  mngUSTAVKI[];//MAX_MNGUSTAVKI+3];
...
int zapusk_permanent=0;//запуск перманентного таймера
...
LineMenuItem sub_menu_ust_compPRM1[] =
{
  ...
  { "Hомин ток выключателя",              (void *)&UstCompPRM1, typeMenuProg},
  { "Hомин ток отключения выключателя",   (void *)&UstCompPRM1, typeMenuProg},
  { "Pесурс номин токе выключателя",      (void *)&UstCompPRM1, typeMenuProg},
  { "Pесурс номин токе откл выключателя", (void *)&UstCompPRM1, typeMenuProg},
  { NULL, NULL}
};
/*
//уставки PRM1 номин ток выключателя
...
//уставка Привод ВВ
    tmpUSTAVKI[MNGVID_PRM1_PRIVODVV] = 159;

//выдержка формирователя блокировки Блока включения
    tmpUSTAVKI[MNGVID_PRM1_BLOKIRBLOKVKL] = 162;

//выдержка формирователя удлинения блокировки Блока включения
    tmpUSTAVKI[MNGVID_PRM1_DELAYBLOKIRBLOKVKL] = 163;
...
//уставки PRM1 номин ток выключателя
    tmpUSTAVKI[MNGUST_PRM1_INOM_VV] = 124;
//уставки PRM1 номин ток отключения выключателя
    tmpUSTAVKI[MNGUST_PRM1_INOM_OTKLVV] = 125;
//уставки PRM1 ресурс номин токе выключателя
    tmpUSTAVKI[MNGUST_PRM1_NNOM_VV] = 126;
//уставки PRM1 ресурс номин токе отключения выключателя
    tmpUSTAVKI[MNGUST_PRM1_NNOM_OTKLVV] = 127;
  }//if
...
*/
  ...
  setTimerPorog(tmpOffsetT+ TIMER_RELEBO,// 22);
    getNativUstMngUSTAVKI(idxC, MNGUST_PRM1_BO));
   //таймер БB
...
//уставка Привод ВВ
    getNativUstMngUSTAVKI(idxC, MNGVID_PRM1_PRIVODVV));

//выдержка формирователя блокировки Блока включения
  setTimerPorog(tmpOffsetT+ TIMER_BLOKIRBLOKVKL_PRM1,
    getNativUstMngUSTAVKI(idxC, MNGVID_PRM1_BLOKIRBLOKVKL));

//выдержка формирователя удлинения блокировки Блока включения
  setTimerPorog(tmpOffsetT+ TIMER_DELAYBLOKIRBLOKVKL_PRM1,
    getNativUstMngUSTAVKI(idxC, MNGVID_PRM1_DELAYBLOKIRBLOKVKL));
...
/*
//уставки PRM1 номин ток выключателя
#define MNGUST_PRM1_INOM_VV      49
//уставки PRM1 номин ток отключения выключателя
#define MNGUST_PRM1_INOM_OTKLVV  50
//уставки PRM1 ресурс номин токе выключателя
#define MNGUST_PRM1_NNOM_VV      51
//уставки PRM1 ресурс номин токе отключения выключателя
#define MNGUST_PRM1_NNOM_OTKLVV  52
*/
...
//уставки PRM1 ресурс номин токе выключателя
int ust_NNOM_VV = mngUSTAVKI[offsetIOCmdPRM1 + MNGUST_PRM1_NNOM_VV];
//уставки PRM1 ресурс номин токе отключения выключателя
int ust_NNOM_OTKLVV = mngUSTAVKI[offsetIOCmdPRM1 + MNGUST_PRM1_NNOM_OTKLVV];
//номин ток отключения выключателя
int ust_INOM_OTKLVV = mngUSTAVKI[offsetIOCmdPRM1 + MNGUST_PRM1_INOM_OTKLVV];
//уставки PRM1 номин ток выключателя
int ust_INOM_VV = mngUSTAVKI[offsetIOCmdPRM1  + MNGUST_PRM1_INOM_VV];

                                                      //уставки PRM1 ресурс номин токе выключателя
   float tmp_1 =  (float) ust_NNOM_VV/
                  (float) ust_NNOM_OTKLVV;
   tmp_1  = log10f(tmp_1);

                  //номин ток отключения выключателя
   float tmp_2 =  (float) ust_INOM_OTKLVV/
                  //уставки PRM1 номин ток выключателя
                  (float) ust_INOM_VV;
    tmp_2  = log10f(tmp_2);

    float K_resurs = tmp_1/tmp_2;
...
//при токе большем, чем I откл номин весовой коэф равен
    if (Imax >=  ust_INOM_OTKLVV) {
//      N = (unsigned int) ceilf((float)current_settings_prt.setpoint_r_kom_st_Inom/(float)current_settings_prt.setpoint_r_kom_st_Inom_vymk);
        N = (unsigned int) ceilf(tmp_1);//(float) mngUSTAVKI[offsetIOCmdPRM1 + MNGUST_PRM1_NNOM_VV]/
                                 //(float));
//    } else if (max_faze_current_vv <= current_settings_prt.setpoint_Inom) {
      } else if (Imax <= ust_INOM_VV) {
      N = 1;
    } else {
      float I_div_Inom = (float) Imax/ ust_INOM_VV;
//(float)max_faze_current_vv / (float)current_settings_prt.setpoint_Inom;
      float lg_I_div_Inom = log10f(I_div_Inom);
      float lg_N = K_resurs * lg_I_div_Inom;
   ...
   return N;
}//wagkoef Wag_Koef_ResursVV(int Imax)
