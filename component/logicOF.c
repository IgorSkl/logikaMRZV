...
//  TRGL_EL(pusk2OF5, pusk2OF5_old, pusk2OF5_trg);
  tmp1 =  pusk1OF5 | pusk2OF | IO_LOGIC(IOCMD_IN_OF5);//ОФункция 5
...
//  TRGL_EL(pusk2OF15, pusk2OF15_old, pusk2OF15_trg);
  tmp1 =  pusk1OF15 | pusk2OF15 | IO_LOGIC(IOCMD_IN_OF15);//ОФункция 15
...
//  TRGL_EL(pusk2OF16, pusk2OF16_old, pusk2OF16_trg);
  tmp1 =  pusk1OF16 | pusk2OF16 | IO_LOGIC(IOCMD_IN_OF16);//ОФункция 16
...
//int setOT1DV_trg = 0;
//int resOT1DV_trg = 0;
  //int setOT1DV = IO_LOGIC(IOCMD_BKL_OT1);//Set ОT1 DV
//  int resOT1DV = IO_LOGIC(IOCMD_OTKL_OT1);//Res ОT1 DV
...
//  if(setOT1DV) OTout[0] = 1;//Выходы OT
//  if(resOT1DV) OTout[0] = 0;//Выходы OT
  //int resOT1DV = IO_LOGIC(IOCMD_OTKL_OT1);//Res ОT1 DV
  int set_tmp  = pusk1OT1 | pusk2OT1 | IO_LOGIC(IOCMD_BKL_OT1);//Set ОT1 DV
  int res_tmp  = rst1OT1  | rst2OT1  | IO_LOGIC(IOCMD_OTKL_OT1);//Res ОT1 DV
  if(set_tmp) OTout[0] = 1;//Выходы OT
  if(res_tmp) OTout[0] = 0;//Выходы OT
...
int rst1OT2_trg = 0;
int rst2OT2_trg = 0;
  int setOT2DV = IO_LOGIC(IOCMD_BKL_OT2);//Set ОT2 DV
  int resOT2DV = IO_LOGIC(IOCMD_OTKL_OT2);//Res ОT2 DV
  ...
  if(pusk1OT2_trg | pusk2OT2_trg | setOT2DV) OTout[1] = 1;//Выходы OT
  if(rst1OT2_trg  | rst2OT2_trg  | resOT2DV)  OTout[1] = 0;//Выходы OT
*/
  set_tmp  = pusk1OT2 | pusk2OT2 | IO_LOGIC(IOCMD_BKL_OT2);//Set ОT2 DV
  res_tmp  = rst1OT2  | rst2OT2  | IO_LOGIC(IOCMD_OTKL_OT2);//Res ОT2 DV
  if(set_tmp) OTout[1] = 1;//Выходы OT
  if(res_tmp) OTout[1] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT3 | pusk2OT3 | IO_LOGIC(IOCMD_BKL_OT3);//Set ОT3 DV
  res_tmp  = rst1OT3  | rst2OT3  | IO_LOGIC(IOCMD_OTKL_OT3);//Res ОT3 DV
  if(set_tmp) OTout[2] = 1;//Выходы OT
  if(res_tmp) OTout[2] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT4 | pusk2OT4 | IO_LOGIC(IOCMD_BKL_OT4);//Set ОT4 DV
  res_tmp  = rst1OT4  | rst2OT4  | IO_LOGIC(IOCMD_OTKL_OT4);//Res ОT4 DV
  if(set_tmp) OTout[3] = 1;//Выходы OT
  if(res_tmp) OTout[3] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT5 | pusk2OT5 | IO_LOGIC(IOCMD_BKL_OT5);//Set ОT5 DV
  res_tmp  = rst1OT5  | rst2OT5  | IO_LOGIC(IOCMD_OTKL_OT5);//Res ОT5 DV
  if(set_tmp) OTout[4] = 1;//Выходы OT
  if(res_tmp) OTout[4] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT6 | pusk2OT6 | IO_LOGIC(IOCMD_BKL_OT6);//Set ОT6 DV
  res_tmp  = rst1OT6  | rst2OT6  | IO_LOGIC(IOCMD_OTKL_OT6);//Res ОT6 DV
  if(set_tmp) OTout[5] = 1;//Выходы OT
  if(res_tmp) OTout[5] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT7 | pusk2OT7 | IO_LOGIC(IOCMD_BKL_OT7);//Set ОT7 DV
  res_tmp  = rst1OT7  | rst2OT7  | IO_LOGIC(IOCMD_OTKL_OT7);//Res ОT7 DV
  if(set_tmp) OTout[6] = 1;//Выходы OT
  if(res_tmp) OTout[6] = 0;//Выходы OT
...
*/
  set_tmp  = pusk1OT8 | pusk2OT8 | IO_LOGIC(IOCMD_BKL_OT8);//Set ОT8 DV
  res_tmp  = rst1OT8  | rst2OT8  | IO_LOGIC(IOCMD_OTKL_OT8);//Res ОT8 DV
  if(set_tmp) OTout[7] = 1;//Выходы OT
  if(res_tmp) OTout[7] = 0;//Выходы OT
