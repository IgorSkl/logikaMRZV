...
hldGoosBWrp.size_GoosB = SIZE_GOOS_B;//+sizeof(BigObjCMUnitDsc);
  ...
  //Reset Connect State
   ...
   ((TRBaseCTpuUnitDsc*)pv)->SizeAppObj = hldGoosBWrp.size_GoosB + sizeof(BigObjCMUnitDsc);//
...
if (unnV1.uchAr[0]== 0)
{
	...
	if( i<= (long)(SIZE_GOOS_B))
	lID = ((LDCIDsc*)pvLDC)->uchTR_C;
	else
	lID = (SIZE_GOOS_B);// - (((RVBaseCTpuUnitDsc*)pv)->ulRvCount);//For Exlude buf overrun
  ...
  //Clear All Ses Data
	...
	do someone wrongholderRVTotMeasRSOCTpuUnit.RvCnHldr.ConState         = 0;
	...
	i = (((LDCIDsc*)pvLDC)->NumComSes)>>4;
		if (i== (((RVBaseCTpuUnitDsc*)pv)->NumComSes) )
		{
			...
			if (lID >= (long)(sLV.shSizeAppObj))
			{
				...
				if ( (lID == (long)(sLV.shSizeAppObj)) &&i)
				{
					//Activate App Func
					AppReqReceiveGoosB++;
...
long CheckGoosBHldCs( void* pGoosBHldDsc) //void*)&holderGoosBIciCopy);
{
register long i;
//struct 
//	{
...
long CalcGoosBHldCs( void* pGoosBHldDsc)//void*)&holderGoosBIciCopy);
{
register long i;
