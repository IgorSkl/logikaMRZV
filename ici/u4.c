...
void Lpc3240_irq_handler (void);
extern PFV irq_func_ptrs[IRQ_END_OF_INTERRUPTS];

/* Prototype for external IRQ handler */
//.void lpc32xx_irq_handler(void);
extern __irq __arm void lpc32xx_irq_handler (void);

/*
__irq __arm void lpc32xx_irq_handler (void)
{
register unsigned long lVr = 0;
register int i = 0;
PFV	pfv = (PFV)0;////(PFV)
...
void StpInitTimers(void)
{
  ...
  //.int_install_arm_vec_handler(IRQ_VEC, (PFV) Lpc3240_irq_handler);
  ...
  timer0dev = Timer_open(TIMER_CNTR0, 0);//.timer_open(TIMER_CNTR0, 0);
  timer1dev = Timer_open(TIMER_CNTR1, 0);//.timer_open(TIMER_CNTR1, 0);
