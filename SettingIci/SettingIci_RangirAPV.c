...
//Пуск АПВ от ДВ 
 if(isActivCmdDV(START_AFU_DI_IEPRF_ONB, numDV))//проверка активности команды на ДВ
      SetCmdToUVV(APV_COMP1,  1, IOCMD2_APV, nDV);//отранж 
//Блокировка АПВ ОТ ДВ
 if(isActivCmdDV(BLOCK_AFU_DI_IEPRF_ONB, numDV))//проверка активности команды на ДВ
      SetCmdToUVV(APV_COMP1,  1, IOCMD1_APV, nDV);//отранж 
...
//APV1
 if(isActivCmdRele(ARCL1_OEPRF_ONB, numRele))//проверка активности команды на Реле
      SetCmdToUVV(APV_COMP1,  1, IOCMD4_APV, nRele);//отранж 
//APV2
 if(isActivCmdRele(ARCL2_OEPRF_ONB, numRele))//проверка активности команды на Реле
      SetCmdToUVV(APV_COMP1,  1, IOCMD5_APV, nRele);//отранж 
